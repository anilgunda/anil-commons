...
public class HiveSchemaBuilder {
	...
	private String getSchema(Schema schema) {
		...
		String columnsSchema = columnSchemaBuilder.getColumnsSchema(schema.getColumnList(),SchemaType.HIVE.getType());
	...
	public Set<String> getSchema(Map<String, Schema> jsonSchemas){
		...
		jsonSchemas.forEach((schemaName, jsonSchema) -> {
			...
			System.out.println("hive schema name :"+schemaName+" hive schema : "+schema);
		});
		return schemas;
	}
}
